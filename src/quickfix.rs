...
/// Parses an `'errorformat'` `%e` end-line number
/// (`qf_parse_fmt_e`).
pub fn qf_parse_fmt_e(
    matched: Option<&[u8]>,
    fields: &mut QffieldsT,
) -> i32 {
    let Some(value) = qf_parse_atol_match(matched) else {
...
/// Parses an `'errorformat'` `%k` end-column number
/// (`qf_parse_fmt_k`).
pub fn qf_parse_fmt_k(
    matched: Option<&[u8]>,
    fields: &mut QffieldsT,
) -> i32 {
    let Some(value) = qf_parse_atol_match(matched) else {
...
/// Set quickfix/location-list entries from a List property
/// (`qf_setprop_items`).
///
/// # Safety
...
unsafe fn qf_setprop_items(
    ...
    let title = unsafe { (&(*qi).qf_lists)[idx].qf_title.clone() };
    unsafe {
        qf_add_entries(
            qi,
            qf_idx,
...
/// remain valid. Touches global quickfix id, callback/security,
/// tab/window, and Dict/List state.
#[allow(dead_code)]
unsafe fn qf_set_properties(
    ...
    let mut newlist =
        action == b' ' || qf_stack_empty(unsafe { qi.as_ref() });
    let mut qf_idx = qf_setprop_get_qfidx(
        unsafe { &*qi },
        unsafe { &mut *what },
        action,
        &mut newlist,
    ...
    if let Some(item) =
        crate::eval::typval::tv_dict_find(Some(unsafe { &mut *what }), b"title")
    {
    ...
    if let Some(item) =
        crate::eval::typval::tv_dict_find(Some(unsafe { &mut *what }), b"context")
    {
        let qfl = unsafe {
            std::ptr::addr_of_mut!((&mut (*qi).qf_lists)[idx])
        };
        retval = unsafe { qf_setprop_context(&mut *qfl, item) };
    }
    if let Some(item) =
        crate::eval::typval::tv_dict_find(Some(unsafe { &mut *what }), b"idx")
    {
        ...
        retval = unsafe { qf_setprop_curidx(qi, qfl, item) };
    }
    if let Some(item) = crate::eval::typval::tv_dict_find(
        ...
        retval = unsafe { qf_setprop_qftf(&mut *qfl, item) };
    }

    if newlist || retval == crate::vim_defs::OK {
        let qfl = unsafe {
            std::ptr::addr_of_mut!((&mut (*qi).qf_lists)[idx])
        };
...
/// `getqflist([{what}])` (`f_getqflist`).
///
/// # Safety
/// Forwarded from [`get_qf_loc_list`].
pub unsafe fn f_getqflist(
    argvars: &[crate::eval::typval_defs::TypvalT],
    rettv: &mut crate::eval::typval_defs::TypvalT,
) {
    let unknown = crate::eval::typval_defs::TypvalT::default();
    let what = argvars.first().unwrap_or(&unknown);
...
mod tests {
    ...
    #[test]
    fn qf_parse_fmt_e_parses_the_end_line_number() {
        let mut fields = QffieldsT::default();
        assert_eq!(
            qf_parse_fmt_e(Some(b"45"), &mut fields),
            qf_status::QF_OK
        );
        assert_eq!(fields.end_lnum, 45);
    }

    #[test]
    fn qf_parse_fmt_e_rejects_a_missing_match_without_changing_end_line() {
        let mut fields = QffieldsT {
            end_lnum: 9,
            ..Default::default()
        };
    ...
    #[test]
    fn qf_parse_fmt_k_parses_the_end_column_number() {
        let mut fields = QffieldsT::default();
        assert_eq!(
            qf_parse_fmt_k(Some(b"29"), &mut fields),
            qf_status::QF_OK
        );
        assert_eq!(fields.end_col, 29);
    }

    #[test]
    fn qf_parse_fmt_k_rejects_a_missing_match_without_changing_end_column() {
        let mut fields = QffieldsT {
            end_col: 6,
            ..Default::default()
        };
        assert_eq!(qf_parse_fmt_k(None, &mut fields), qf_status::QF_FAIL);
        assert_eq!(fields.end_col, 6);
    }
    ...
    #[test]
    fn getqflist_builtin_returns_the_current_entry_list() {
        let _lock = crate::globals::global_state_test_lock();
        let _ql_info = QlInfoGuard::save();
        *unsafe { QL_INFO.get_mut() } = Some(crate::types_defs::QfInfoT {
            qf_listcount: 1,
            qf_lists: vec![QfListT {
                qf_entries: vec![QflineT { qf_lnum: 6, ..Default::default() }],
                ..Default::default()
            }],
            ..Default::default()
        });
    ...
    #[test]
    fn getqflist_builtin_accepts_a_property_request_dict() {
        let _lock = crate::globals::global_state_test_lock();
        let _ql_info = QlInfoGuard::save();
        *unsafe { QL_INFO.get_mut() } = Some(crate::types_defs::QfInfoT {
            qf_listcount: 1,
    ...
    #[test]
    fn qf_set_properties_updates_existing_metadata_and_changedtick() {
        let _lock = crate::globals::global_state_test_lock();
        let _first_tabpage = unsafe {
            crate::globals::GlobalFieldGuard::install(
                |globals| &mut globals.first_tabpage,
        ...
        crate::eval::typval::tv_dict_add_nr(what_ref, b"context", 7);
        crate::eval::typval::tv_dict_add_nr(what_ref, b"idx", 2);
        crate::eval::typval::tv_dict_add_str(
            what_ref,
            b"quickfixtextfunc",
        ...
        let qfl = &mut qi.qf_lists[0];
        assert_eq!(qfl.qf_title.as_deref(), Some(&b"new"[..]));
        assert_eq!(qfl.qf_index, 2);
        assert_eq!(qfl.qf_changedtick, 1);
        assert!(matches!(
            qfl.qf_ctx.as_deref().map(|context| &context.value),
            Some(crate::eval::typval_defs::TypvalValue::Number(7))
        ));
        assert_eq!(
            qfl.qf_qftf_cb.kind(),
            crate::eval::typval_defs::CallbackType::Funcref
    ...
    #[test]
    fn qf_set_properties_creates_and_marks_a_new_list() {
        let _lock = crate::globals::global_state_test_lock();
        let _ids = LastQfIdGuard::new();
        let _first_tabpage = unsafe {
            crate::globals::GlobalFieldGuard::install(
    ...
    #[test]
    fn qf_set_properties_rejects_an_unknown_target_list() {
        let _lock = crate::globals::global_state_test_lock();
        let mut qi = crate::types_defs::QfInfoT {
            qf_listcount: 1,
            qf_lists: vec![QfListT::default()],
        ...
        let qi_ptr = std::ptr::from_mut(&mut qi);
        let what = crate::eval::typval::tv_dict_alloc();
        crate::eval::typval::tv_dict_add_nr(unsafe { &mut *what }, b"nr", 99);
        crate::eval::typval::tv_dict_add_str(
            unsafe { &mut *what },
            b"title",
            Some(b"ignored"),
    ...
    fn items_property(
        list: *mut crate::eval::typval_defs::ListT,
    ) -> *mut crate::eval::typval_defs::DictitemT {
        unsafe { crate::eval::typval::tv_list_ref(list) };
        let item = crate::eval::typval::tv_dict_item_alloc(b"items");
        unsafe {
            (*item).di_tv.value =
                crate::eval::typval_defs::TypvalValue::List(list);
        }
    ...
    #[test]
    fn qf_setprop_items_replaces_entries_and_preserves_the_title() {
        let _lock = crate::globals::global_state_test_lock();
        let mut qi = crate::types_defs::QfInfoT {
            qf_listcount: 1,
            qf_lists: vec![QfListT {
        ...
        unsafe {
            crate::eval::typval::tv_list_append_dict(
                list,
                batch_item_dict(0, 9, 1, Some(true)),
            )
        };
        let item = items_property(list);

        assert_eq!(
            unsafe { qf_setprop_items(qi_ptr, 0, item, b'r') },
            qf_status::QF_OK
        );
        assert_eq!(qi.qf_lists[0].qf_count(), 1);
        assert_eq!(qi.qf_lists[0].qf_entries[0].qf_lnum, 9);
        assert_eq!(qi.qf_lists[0].qf_title.as_deref(), Some(&b"kept"[..]));
    ...
    #[test]
    fn qf_setprop_items_maps_new_list_action_to_append() {
        let _lock = crate::globals::global_state_test_lock();
        let mut qi = crate::types_defs::QfInfoT {
            qf_listcount: 1,
            qf_lists: vec![QfListT::default()],
        ...
        assert_eq!(
            unsafe { qf_setprop_items(qi_ptr, 0, item, b' ') },
            qf_status::QF_OK
        );
        assert_eq!(qi.qf_listcount, 1);
        assert_eq!(qi.qf_lists[0].qf_count(), 1);
    ...
    #[test]
    fn qf_setprop_items_rejects_a_non_list_value() {
        let mut qi = crate::types_defs::QfInfoT {
            qf_listcount: 1,
            qf_lists: vec![QfListT::default()],
            ..Default::default()
        ...
        assert_eq!(qi.qf_lists[0].qf_count(), 0);
        unsafe { crate::eval::typval::tv_dict_item_free(item) };
    }
}
